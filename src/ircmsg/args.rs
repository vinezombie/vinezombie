...
impl<'a> Args<'a> {
    ...
    /// Adds an [`Arg`] to the argument array.
    ///
    /// The argument will be added to the end of the argument array
    /// unless the last argument is long in which case it will be added just before it.
    ...
    pub fn add_last<'b: 'a>(&mut self, s: impl Into<Line<'b>>) {
        let s = s.into();
